...
# model config
# =========================
CKPT_PATH = "policy_img_state_stack4_multi_k3_a.pt"
IMG_SIZE = 96
STACK = 4
IN_CH = 3 * STACK
OUT_DIM = 7
...
KEYS = ["UP", "DOWN", "LEFT", "RIGHT", "ALT", "SHIFT", "A"]
...
def update_keys(prob_raw: np.ndarray, now: float):
    ...
    if float(prob[6]) >= THR_TAP:
        current_active.add("A")
    ...
    for idx, k in [(4, "ALT"), (5, "SHIFT"), (6, "A")]:
        want = float(prob[idx]) >= THR_TAP
        if want and k not in pressed_tap:
            pyautogui.keyDown(PYA_KEYS[k])
            pressed_tap.add(k)
        elif (not want) and k in pressed_tap:
            pyautogui.keyUp(PYA_KEYS[k])
            pressed_tap.remove(k)
...
def main():
    ...
    WIN_TITLE = "Live Preview (policy -> pyautogui hold-keys, multi state + A)"
