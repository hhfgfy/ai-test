...
LABELS_PATH = os.path.join(SAVE_DIR, "labels_a.npy")
FPS = 20
MAX_SAMPLES = 5000
PREVIEW_MAX_WIDTH = 1200

pressed = set()
A_KEYS = {keyboard.KeyCode.from_char("a"), keyboard.KeyCode.from_char("A")}
...
def get_action_vec():
    # [UP, DOWN, LEFT, RIGHT, ALT, SHIFT, A]
    ...
    a_btn = 1 if any(k in pressed for k in A_KEYS) else 0

    return np.array([up, down, left, right, alt, shift, a_btn], dtype=np.uint8)
    ...
    try:
        ...
        try:
            if len(labels) > 0:
                ...
                print("labels saved:", LABELS_PATH, labels_arr.shape)
            else:
                print("No labels collected (0 samples). labels not saved.")
        except Exception as e:
            print("Failed to save labels:", repr(e))
