...
LABELS_PATH = "data/labels_a.npy"
STATE_PATH  = "data/state_multi_k3.npy"
OUT_PATH = "policy_img_state_stack4_multi_k3_a.pt"
...
KEY_DIM = 7
MAX_MONSTERS = 3
STATE_DIM = 2 + 4 * MAX_MONSTERS  # [px,py] + K*[mx,my,dx,dy]
...
class ImitationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for k in range(STACK):
            ...
            if j < 0:
                j = 0
            frames.append(load_img(self.frame_paths[j]))
        x_img = torch.cat(frames, dim=0)

        x_state = torch.from_numpy(self.states[idx]).float()
...
def main():
    ...
    frame_paths, labels, states = frame_paths[:n], labels[:n], states[:n]
    ...
    model = ImgStatePolicyStack(out_dim=KEY_DIM, state_dim=STATE_DIM, in_ch=IN_CH).to(device)

    opt = torch.optim.Adam(model.parameters(), lr=LR)
    loss_fn = nn.BCEWithLogitsLoss()

    print(f"N={n} | train={len(train_ds)} val={len(val_ds)} | device={device} | stack={STACK} | state_dim={STATE_DIM} | key_dim={KEY_DIM}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x_img, x_state, y in train_dl:
            ...
            loss = loss_fn(model(x_img, x_state), y)
        ...
        with torch.no_grad():
            for x_img, x_state, y in val_dl:
                x_img, x_state, y = x_img.to(device), x_state.to(device), y.to(device)
                va_loss += loss_fn(model(x_img, x_state), y).item() * y.size(0)
    ...
    torch.save(
        {"model_state": model.state_dict(), "img_size": IMG_SIZE, "uses_state": True, "stack": STACK, "state_dim": STATE_DIM, "key_dim": KEY_DIM},
        OUT_PATH
    )
